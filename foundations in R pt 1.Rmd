...
Running this code tells us the statement is false. (**Note**: this is because the phrase on the left 
...
```{r}
animalDat <- cbind.data.frame(animals, animalCookies)
animalDat
rowAnimalDat <- rbind.data.frame(animals, animalCookies)
rowAnimalDat
```
...
```{r}
animalDat[, 1] #first column
animalDat[1 ,] #first row
...
```{r}
tvShows<- c("brooklynn99", "friends", "gilmore girls", "vice principals")
tvRatings<- c(10, 10, 8, 10)
```

3.  Make these vectors into a data frame.

```{r}
tvDat<- cbind.data.frame(tvShows, tvRatings)
```
...
#selective about row, but want all the data across columns
```
...
**Use command, shift, M to create pipe**
...
```{r}
mtcars %>% filter(gear==5) %>% select(mpg, hp)
```
...
```{r}
under100gm<- ChickWeight %>% filter(weight<100)
```
...
```{r}
range(mtcars$mpg) #subtract two values
```
