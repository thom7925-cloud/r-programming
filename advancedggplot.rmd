...
```{r}
set.seed(137)
stateUniversity<- sample(c("UMN", "UMD", "UMich", "KU", "UGA", "NYU", "URI", "Rutgers", "UPenn", "OSU", "PennState", "UDel", "UConn", "UCLA", "UC Riverside", "UC Santa Barbara", "UC Merced", "NDSU", "USC", "UNC"), 100, replace = TRUE)
...
What are interactions?
  ...
  -used in regression and ANOVA (which test the same thing using different math/assumptions)
...
```{r}
subset(bobaDat, bobaDat$schoolClimate== "cool"&
         bobaDat$studentInterest=="high") ##checking data to ensure this combination doesn't exist
```
...
ggplot(summaryBoba, aes(schoolClimate, mean, fill = studentInterest))+ 
  ...
  geom_errorbar(aes(ymin= mean-stderr, ymax = mean+stderr), position= position_dodge2(), width= 1)
```

Alternatively, could use facet wrap with boxplots
```{r}
ggplot(bobaDat, aes(schoolClimate, boba, fill= studentInterest))+
  geom_boxplot()+
