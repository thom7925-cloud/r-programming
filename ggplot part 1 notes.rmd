...
diamonds %>% 
  ...
  dplyr::summarize(meanPrice= mean(price), sd(price))
```
...
```{r}
data(penguins)
fliplengthBySpecies<- penguins %>% 
  dplyr:: group_by(species) %>% 
  dplyr:: summarize(mean= mean(flipper_len, na.rm = TRUE), 
                    sd= sd(flipper_len, na.rm = TRUE), 
                    se= sd(flipper_len, na.rm = TRUE)/sqrt(nrow(penguins)), 
                    lowerCI= (mean(flipper_len, na.rm = TRUE) - .95*sd(flipper_len, na.rm = TRUE))/sqrt(nrow(penguins)))
```
...
```{r}
ggplot(penguins, aes(bill_len))+ geom_histogram(bins=20)
```
2. Include a theme to change the appearance of your graph.
```{r}
ggplot(penguins, aes(bill_len))+ geom_histogram(bins=20)+ theme_classic()
```

3. Create a histogram with orange bars and purple lines. 
```{r}
ggplot(penguins, aes(bill_len))+ geom_histogram(bins= 20, color= "purple", fill= "orange")+ theme_classic()
```

4. **Bonus** Create a histogram with the worst color combination you can think of. 
```{r}
ggplot(penguins, aes(bill_len))+ geom_histogram(bins= 20, color= "darkorange4", fill= "yellow4")+ theme_classic()
```
...
```{r}
ggplot(diamonds, aes(clarity))+ geom_bar(fill= "lightgreen")+ theme_bw()
```
...
ggplot(diamonds, aes(clarity))+ 
  geom_bar(color= "black", fill= "lightgreen") + 
  theme_classic() + 
  labs(title= "Graph of Diamond Clarity", x= "Diamond Clarity", y= "Frequency")
```
...
```{r}
ggplot(priceByColor, aes(color, price))+ 
  geom_bar(stat="identity", fill="aquamarine", alpha= .4)+ 
  geom_errorbar(aes(ymin = price - se, ymax = price + se, width = .5))+
  theme_classic() ##alpha 0-1 changes transparency
```
...
ggplot(priceByColor, aes(color, price))+ 
  ...
  geom_errorbar(aes(ymin = price - se, ymax = price + se, width = .5))+
  theme_classic() +
  geom_hline(yintercept = mean(diamonds$price), linewidth = 1, linetype= "dashed", color= "purple")
...
ggplot(penguins, aes(species))+
  geom_bar(color= "aquamarine", fill= "darkblue")+
  theme_classic()
```
...
ggplot(penguins, aes(species))+
  geom_bar(color= "aquamarine", fill= "darkblue")+
  theme_classic()+
  labs(title= "Penguins", x= "Species", y= "Frequency")
```
...
ggplot(diamonds, aes(color, fill= cut))+ 
  ...
  theme_classic()   #categorical variables
```
...
#jitter()
```{r}
ggplot(mtcars, aes(cyl, gear))+ geom_jitter() #only add to other graphs for a sense of variability
```

### Practice
- make a scatter plot of bill length and bill depth. 
```{r}
ggplot(penguins, aes(bill_len, bill_dep))+ geom_point()
```

- modify the graph to include at least 2of the style elements we talked about
```{r}
ggplot(penguins, aes(bill_len, bill_dep))+ geom_point(color= "lightpink", size= 2, shape= 10)
```
...
```{r}
ggplot(diamonds, aes(carat, price))+geom_point()+geom_smooth(method = "lm", linewidth=1, color="orange", linetype= "dashed")
```
...
```{r}
ggplot(mtcars, aes(mpg, hp))+ geom_point()+ geom_smooth(method= "lm", se= FALSE) #se=FALSE to remove conf interval shading
```
...
```{r}
ggplot(penguins, aes(body_mass, flipper_len))+ geom_point()
```
2. Change the color of the points to your favorite color. 
```{r}
ggplot(penguins, aes(body_mass, flipper_len))+ geom_point(color= "darkgreen")
```
3. Change the shape of the points. 
```{r}
ggplot(penguins, aes(body_mass, flipper_len))+ geom_point(color= "darkgreen", shape= 6)
```
4. Add a line of best fit to your scatter plot. 
```{r}
ggplot(penguins, aes(body_mass, flipper_len))+ geom_point(color= "darkgreen", shape= 6)+ geom_smooth(method= "lm", color= "black")
```
...
```{r}
ggplot(ChickWeight, aes(Time, weight, color=Chick))+ geom_line()
```
...
```{r}
ggplot(penguins, aes(flipper_len))+ geom_boxplot()
```

2. Create boxplots of flipper length for each species.
```{r}
ggplot(penguins, aes(flipper_len, species))+ geom_boxplot()
```

3. **Bonus** play around with fill and color. What changes about your boxplots? 
```{r}
ggplot(penguins, aes(flipper_len, species))+ 
  geom_boxplot(color= "lightpink", fill= "darkgreen")
```

###Best and Worst Graphs
```{r}
mentalDat <- read.csv("~/Downloads/students_mental_health_survey fall 2025.csv")
```

```{r}
ggplot(mentalDat, aes(cortisolLevel, stressLevel))+
  geom_point(color = "khaki4", size= 5, shape= 12)+
   theme(panel.background = element_rect(fill= "olivedrab4"))
```

```{r}
ggplot(mentalDat, aes(as.factor(cortisolLevel), stressLevel))+ 
  geom_boxplot(color= "plum", fill= "navy")+ 
  theme_classic()+
  labs(title= "Stress by Cortisol", x= "Cortisol Level", y= "Stress Level")
```
...
```{r}
ggplot(diamonds, aes(price, fill=color))+ 
  geom_histogram(position= position_dodge(), bins= 12)+
  scale_fill_manual(values = c("#D7F2BA", "#0D2818", "#BC4749", "#058C42", "#FEC196", "#044B7F", "#011627"))
```
`scale_color_manual(values = #)`; 
```{r}
ggplot(diamonds, aes(price, carat, color=cut))+ 
  geom_point()+ 
  scale_color_manual(values = c("#D7F2BA", "#0D2818", "#BC4749", "#058C42", "#FEC196", "#044B7F", "#011627"))
```
...
ggplot(diamonds, aes(price, fill=color))+ 
  geom_histogram(position= position_dodge(), bins= 12)+
  scale_fill_viridis(discrete = TRUE, option = "plasma") ##use discrete when variable is categorical 
```
...
```{r}
ggplot(penguins, aes(bill_len, fill=island))+ geom_histogram(position = position_dodge())
```

2. In some way, include species as a third variable in this graph. 
```{r}
ggplot(penguins, aes(island, bill_len, color = species))+ geom_point()
```
...
```{r}
ggplot(penguins, aes(bill_len, bill_dep, color= species))+ geom_point()
```