...
summary(mtcars$hp)
#summaries look different for factor (qualitative stats) and numeric (quantitative stats)
```
...
tenToHundred<- 10:100
number<- NA
for(i in 1:length(tenToHundred)){
  number[i]<- tenToHundred[i]*4
}
for(i in 1:length(tenToHundred)){
  print(tenToHundred[i]*4)
}
```
...
for(i in 1:nrow(mtcars)) {
    mtcars$goodMPG[i]<- mtcars$mpg[i] > 25
}
```
...
```{r}
mtcars$mpgAbove25<- ifelse(mtcars$mpg>25, "good mileage", "bad mileage")
...
```{r}
data("ChickWeight")
ChickWeight$heavy<- NA
ChickWeight$heavy<- ifelse(ChickWeight$weight>100, "heavy", "light")
```
for(){
  if(){
...
for(i in 1:nrow(penguins)) {
  ...
  if(penguins$island[i]=="Biscoe"){
      penguins$studentFavorite[i]<- "Saywer"
  } 
